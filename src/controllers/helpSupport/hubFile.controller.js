...
import * as hubFileService from '../../services/helpSupport/hubFile.service.js';
...
const createFolder = catchAsync(async (req, res) => {
  ...
  const folder = await hubFileService.createFolder(folderBody);
  res.status(httpStatus.CREATED).send(hubFileService.formatHubItem(folder));
});
...
const createFile = catchAsync(async (req, res) => {
  ...
  const file = await hubFileService.createFile(fileBody);
  res.status(httpStatus.CREATED).send(hubFileService.formatHubItem(file));
});
...
const getFolderContents = catchAsync(async (req, res) => {
  ...
  const result = await hubFileService.getFolderContents(req.params.folderId, options);
...
const getRootFolders = catchAsync(async (req, res) => {
  const options = pick(req.query, ['sortBy', 'limit', 'page']);
  const result = await hubFileService.getRootFolders(req.user.id, options);
  res.set('Cache-Control', 'no-store');
  res.send(result);
});
...
const getFolderTree = catchAsync(async (req, res) => {
  ...
  const tree = await hubFileService.getFolderTree(req.user.id, rootFolderId);
  res.send(tree);
});
...
const deleteFolder = catchAsync(async (req, res) => {
  const result = await hubFileService.deleteFolder(req.params.folderId);
...
const deleteFile = catchAsync(async (req, res) => {
  const file = await hubFileService.deleteFile(req.params.fileId);
...
const deleteMultipleItems = catchAsync(async (req, res) => {
  ...
  const result = await hubFileService.deleteMultipleItems(itemIds);
...
const getDashboard = catchAsync(async (req, res) => {
  ...
  // Get root folders
  const rootFolders = await hubFileService.getRootFolders(req.user.id, { ...options, limit: 5 });
  
  // Get recent files
  const recentFiles = await hubFileService.searchItems(
    { userId: req.user.id, type: 'file' },
    { ...options, limit: 10, sortBy: 'createdAt:desc' }
  );
  
  // Get folder tree for navigation
  const folderTree = await hubFileService.getFolderTree(req.user.id);
